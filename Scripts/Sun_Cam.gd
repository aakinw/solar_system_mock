...
#Zoom Settings
@export var min_zoom = 300;
@export var max_zoom = 6000;
	...
	if(current == true):
		...
		if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
			...
			if event is InputEventMouseMotion:
				...
				camera.rotation.x = clamp(camera.rotation.x, deg_to_rad(-90), deg_to_rad(50));

func zoom(delta: float):
	var newZoom = clamp(camera.position.y + zoom_speed * delta * zoom_direction, min_zoom, max_zoom);
